...
\vspace{2mm}
\textbf{Response: we changed ``previsions'' by ``predictions''. There was a mistake on p. 4 because we should refer to the ``second round''. We fixed this and added a note explaining that each team plays 19 matches each round, since the championship has 20 teams. More details on the championship rule are given in Section 4 (Results).} 
...
\hspace{4mm} \textbf{Reviewer:} \textit{\textbf{Section 2: The models: theoretical background.} 
	...
	\vspace{2mm}
	\textbf{Response: We believe that iwhen the model assumptions are different, the only possible comparison are in terms
		of predictive accuracy. In the present study, this comparison is possible since all the models report the probabilities of win, draw or loss of the home team.
		Furthermore, from the perspective of football fans the most important feature of a model is to provide ``good'' predictions.} 
